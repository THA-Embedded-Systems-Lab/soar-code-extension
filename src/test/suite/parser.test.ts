...
suite('Soar Parser Test Suite', () => {
    ...
    suite('Attribute Parsing', () => {
        test('Single attribute on one line', () => {
            ...
            // Check LHS attribute
            const nameAttr = prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.value === 'blocks-world');
            ...
            // Check RHS attribute
            const doneAttr = prod.attributes.find((a: SoarAttribute) => a.name === 'done' && a.value === 'true');
        ...
        test('Multiple attributes on one line', () => {
            ...
            const typeAttr = prod.attributes.find((a: SoarAttribute) => a.name === 'type' && a.value === 'state');
            assert.ok(typeAttr);
        ...
        test('Multiple attributes on multiple lines', () => {
            ...
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.value === 'blocks-world' && a.parentId === 's'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'type' && a.value === 'state' && a.parentId === 's'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'superstate' && a.value === 'nil' && a.parentId === 's'));
        });
        ...
        test('Attribute with single variable value', () => {
            ...
            const opAttr = prod.attributes.find((a: SoarAttribute) => a.name === 'operator' && a.value === '<o>');
        ...
        test('Attribute with multiple variable values on one line', () => {
            ...
            const objectAttrs = prod.attributes.filter((a: SoarAttribute) => a.name === 'object' && a.parentId === 's');
            assert.strictEqual(objectAttrs.length, 3, 'Should have 3 object attributes');

            assert.ok(objectAttrs.find((a: SoarAttribute) => a.value === '<blockA>'), 'Should have <blockA>');
            assert.ok(objectAttrs.find((a: SoarAttribute) => a.value === '<blockB>'), 'Should have <blockB>');
            assert.ok(objectAttrs.find((a: SoarAttribute) => a.value === '<blockC>'), 'Should have <blockC>');
        });
        ...
        test('Attribute with multiple variable values - 4 objects', () => {
            ...
            assert.ok(objectAttrs.find((a: SoarAttribute) => a.value === '<blockB>'), 'Should have <blockB>');
            assert.ok(objectAttrs.find((a: SoarAttribute) => a.value === '<blockC>'), 'Should have <blockC>');
            assert.ok(objectAttrs.find((a: SoarAttribute) => a.value === '<table>'), 'Should have <table>');
        });
        ...
        test('Complex multi-line with multiple variables per attribute', () => {
            ...
            const nameAttrs = prod.attributes.filter((a: SoarAttribute) => a.name === 'name' && a.parentId === 's');
            assert.strictEqual(nameAttrs.length, 1);
            assert.strictEqual(nameAttrs[0].value, 'blocks-world');

            // Check ontop attributes (3 values)
            const ontopAttrs = prod.attributes.filter((a: SoarAttribute) => a.name === 'ontop' && a.parentId === 's');
            ...
            const desiredAttrs = prod.attributes.filter((a: SoarAttribute) => a.name === 'desired' && a.parentId === 's');
            assert.strictEqual(desiredAttrs.length, 1);
            assert.strictEqual(desiredAttrs[0].value, '<ds>');

            // Check gripper attribute (1 value)
            const gripperAttrs = prod.attributes.filter((a: SoarAttribute) => a.name === 'gripper' && a.parentId === 's');
        ...
        test('Multiple identifiers with attributes', () => {
            ...
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'operator' && a.parentId === 's'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'io' && a.parentId === 's'));

            // Check operator attributes
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.parentId === 'o'));

            // Check io attributes
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'input-link' && a.parentId === 'io'));
        });
        ...
        test('Negated attributes', () => {
            ...
            const nameAttr = prod.attributes.find((a: SoarAttribute) => a.name === 'name' && !a.isNegated);
        ...
        test('Attributes with constant values', () => {
            ...
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.value === 'blocks-world'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'count' && a.value === '5'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'status' && a.value === 'complete'));
        });
        ...
        test('Attributes without values', () => {
            ...
            const opAttr = prod.attributes.find((a: SoarAttribute) => a.name === 'operator' && a.parentId === 's');
        ...
        test('Real world example - initialize-blocks-world', () => {
            ...
            // State attributes
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'operator.name' && a.parentId === 's'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'io.output-link' && a.parentId === 's'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.value === 'blocks-world' && a.parentId === 's'));
            ...
            // Gripper attributes
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'holding' && a.value === 'nothing' && a.parentId === 'g'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'position' && a.value === 'up' && a.parentId === 'g'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'above' && a.value === '<table>' && a.parentId === 'g'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'open' && a.value === 'yes' && a.parentId === 'g'));

            // Block attributes
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.value === 'A' && a.parentId === 'blockA'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'type' && a.value === 'block' && a.parentId === 'blockA'));

            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.value === 'B' && a.parentId === 'blockB'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'type' && a.value === 'block' && a.parentId === 'blockB'));

            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.value === 'C' && a.parentId === 'blockC'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'type' && a.value === 'block' && a.parentId === 'blockC'));

            // Table attributes
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'name' && a.value === 'table' && a.parentId === 'table'));
            assert.ok(prod.attributes.find((a: SoarAttribute) => a.name === 'type' && a.value === 'table' && a.parentId === 'table'));
        });
    });
