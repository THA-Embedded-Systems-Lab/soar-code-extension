...
documents.onDidOpen(async event => {
    ...
    if (!currentProject) {
        try {
            // Extract workspace folder from document URI
            const uri = event.document.uri;
            const workspaceRoot = uri.substring(0, uri.lastIndexOf('/'));
            const projectFile = await projectLoader.findProjectFile(workspaceRoot.replace('file://', ''));
            if (projectFile) {
                currentProject = await projectLoader.loadProject(projectFile);
...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
    ...
    for (const production of soarDoc.productions) {
        if (productionNames.has(production.name)) {
            ...
            diagnostics.push({
                ...
                range: {
                    start: { line: production.nameRange.start.line, character: production.nameRange.start.character },
                    end: { line: production.nameRange.end.line, character: production.nameRange.end.character }
                },
                message: `Duplicate production name '${production.name}' (first defined at line ${first.nameRange.start.line + 1})`,
    ...
    // Send diagnostics
    connection.sendDiagnostics({ uri: textDocument.uri, diagnostics: diagnostics.slice(0, globalSettings.maxNumberOfProblems) });
}
        ...
        if (isPositionInRange(position, production.range)) {
            ...
            if (isPositionInRange(position, production.nameRange)) {
                return {
                    contents: {
                        kind: 'markdown',
                        value: `**${production.type}** \`${production.name}\`\n\n` +
            ...
            for (const [varName, variable] of production.variables) {
                if (isPositionInRange(position, variable.range)) {
                    return {
                        contents: {
                            kind: 'markdown',
                            value: `**Variable** \`<${varName}>\`\n\nReferences: ${variable.references.length + 1}`
                        }
                    };
                }
...
connection.onCompletion((params: TextDocumentPositionParams): CompletionItem[] => {
    ...
    connection.console.log(`Attribute path match: ${attributePathMatch ? attributePathMatch[0] : 'null'}`);
    ...
    if (attributePathMatch && currentProject) {
        ...
        connection.console.log(`Navigating path: ${attributePath} (segments: ${pathSegments.join(', ')})`);
        ...
        connection.console.log(`Path from root not found, searching all vertices for: ${pathSegments[pathSegments.length - 1]}`);
    ...
    if (currentProject && rootAttrMatch) {
        ...
        if (varName) {
            ...
            const bindingPattern = new RegExp(`\\^([a-zA-Z0-9_-]+(?:\\.[a-zA-Z0-9_-]+)*)\\s+<${varName}>`, 'g');
            let bindingMatch;
            ...
            for (const binding of bindings) {
                ...
                if (foundPath) {
                    ...
                    if (targetVertex && targetVertex.type === 'SOAR_ID') {
                        const attributes = projectLoader.getVertexAttributes(currentVertexId, currentProject);
                        connection.console.log(`Adding ${attributes.length} attributes for <${varName}> bound to ${binding}`);
