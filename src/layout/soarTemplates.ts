...
`;
     ...
     */
    static generatePropose(operatorName: string): string {
        return `###
### Propose operator: ${operatorName}
###

sp {propose*${operatorName}
   "Propose ${operatorName} operator"
...
sp {apply*${operatorName}
   "Apply ${operatorName} operator"
   (state <s> ^operator <op>)
   (<op> ^name ${operatorName})
-->
   # Add actions here
   (write (crlf) |Applying ${operatorName}|)
}
`;
    }
     ...
     */
    static generateOperatorFile(operatorName: string): string {
        return `###
### Operator: ${operatorName}
### Generated by Soar VS Code Extension
###

${this.generateElaborations(operatorName)}
${this.generatePropose(operatorName)}
${this.generateApply(operatorName)}
`;
    }
...
`;
    ...
    static generateAllFile(folderName: string, subfolders: string[]): string {
        const pushCommands = subfolders.map(folder => `pushd ${folder}\nsource _all.soar\npopd`).join('\n\n');
        return `###
