...
import { VisualSoarProject, LayoutNode, ProjectContext, hasChildren } from '../server/visualSoarProject';
...
export class LayoutTreeProvider implements vscode.TreeDataProvider<LayoutTreeItem> {
    private _onDidChangeTreeData: vscode.EventEmitter<LayoutTreeItem | undefined | null | void> = new vscode.EventEmitter<LayoutTreeItem | undefined | null | void>();
    readonly onDidChangeTreeData: vscode.Event<LayoutTreeItem | undefined | null | void> = this._onDidChangeTreeData.event;
    ...
    async loadProject(workspaceFolder: vscode.Uri): Promise<void> {
        try {
            ...
            this.refresh();
            vscode.window.showInformationMessage(`Loaded project structure: ${path.basename(projectFile)}`);
        } catch (error: any) {
