/**
 * Datamap Operations UI
 *
 * Interactive (VS Code prompt-driven) datamap CRUD flows: attribute
 * add/edit/delete/link with quick-pick and input-box prompts, confirmation
 * dialogs, and notifications. This file is imported only from the
 * extension host (src/extension.ts); the standalone MCP process only ever
 * needs the pure logic in datamapOperations.ts.
 */

import * as vscode from 'vscode';
import { DMVertex, OutEdge, SoarIdVertex, EnumerationVertex } from '../server/visualSoarProject.js';
import { generateVertexId } from '../server/idGeneration.js';
import { DatamapProjectContext, InboundEdgeInfo } from './datamapMetadata.js';
import { DatamapOperations } from './datamapOperations.js';

export class DatamapOperationsUi {
  /**
      ...
      if (vertex.type === 'SOAR_ID') {
        ...
        if (soarIdVertex.outEdges) {
          for (const subEdge of soarIdVertex.outEdges) {
            DatamapOperations.removeVertexRecursive(subEdge.toId, projectContext);
          }
        }
      }
  ...
  /**
   * Helper: Save project to file, surfacing a VS Code error notification on
   * failure (the core DatamapOperations.saveProject only logs to console,
   * since it also runs in the standalone MCP process).
   */
  private static async saveProject(projectContext: DatamapProjectContext): Promise<void> {
    try {
      await DatamapOperations.saveProject(projectContext);
    } catch (error: any) {
