...
- `src/datamap/datamapUsage.ts`
  ...
  - `ARCHITECTURAL_ATTRIBUTES` (exported set: `superstate`, `top-state`, `type`, `impasse`, `choices`, `item`, `item-count`, `non-numeric`, `quiescence`, `attribute`, `io`, `input-link`, `output-link`, `reward-link`, `epmem`, `smem`, `operator`, `name`) is exempt from every bucket; callers can add more via `options.extraExemptAttributes`.
