...
- `src/mcp/soarMcpCore.ts`
  ...
  - `fetchLiveDatamapContext(agent)`: runtime-only datamap source for `agent_runtime_connect_context`/`agent_runtime_get_datamap` — fetches the kernel's own cached copy of the project JSON over SML (`get_datamap`, i.e. `Agent::GetDatamap()`; pushed at agent-load time by `soar_ros::SoarRunner::addAgent` via `Agent::SetDatamap()`), so it needs no filesystem access and always matches whatever project the live agent actually loaded — distinct from `loadDatamapContext`, which reads a `.vsa.json` off disk for the project-file-based dev tools (`datamap_get_tree` and the CRUD tools). Result is cached per agent on the `SoarMcpCore` instance (`liveDatamapCache`, invalidated in `debugDisconnect`) since the fetched JSON can be large.
  - `buildCompactDatamapTree(context, vertexId, maxDepth, ancestry)`: sibling to `buildDatamapTree` for the runtime-only tools — omits `id`/`targetId` (schema-graph-only ids, meaningless to a live `print` query; attribute name paths are the actual bridge to live working memory), keeps `type`/`comment`/`choices`, attribute edges keyed by name rather than a `targetId` pointer, and marks `truncated: true` instead of recursing once `maxDepth` is exhausted so the caller knows to drill down via `agent_runtime_get_datamap`'s `path`.
...
- `agent_runtime_connect_context` — connects exactly like `agent_runtime_connect` (delegates to it unchanged), plus a short agent description (the project's `datamap.purpose`) and a compact, depth-2 datamap tree — both sourced live from the kernel via `fetchLiveDatamapContext`, no filesystem access. Non-fatal on enrichment failure (e.g. agent wasn't loaded from a project file): still returns the base connect result, just without `description`/`datamap`. Built for the explainability chat LLM's system prompt (see `explainability/src/agent/explainLoop.ts`) so it starts a conversation already knowing the agent's role and working-memory shape instead of discovering them via trial-and-error `print` calls.
- `agent_runtime_get_datamap` — runtime-only equivalent of `datamap_get_tree`: no project file, sourced live via `fetchLiveDatamapContext`. Without `path`, returns one depth-limited (default 2) compact tree from the datamap root. With `path` (dot-separated attribute path from the root, e.g. `"operator"` or `"operator.name"`, resolved via the existing `DatamapMetadataCache.findTargetVerticesForPath`), returns the subtree(s) at that path — an array, since the same attribute name can appear under multiple parents. Intended workflow: use this to find where something *should* be in the schema, then confirm with a live `agent_runtime_cli_print_working_memory` pattern query (e.g. `"(S1 ^operator)"`) whether it's actually instantiated right now — this tool never reflects current values, only structure.
...
- Individual Soar CLI command tools (for smaller/local LLMs that need explicit schemas):
  ...
  - `agent_runtime_cli_print_working_memory` / `agent_runtime_cli_print_production` — both wrap the same underlying `print [options] [target]` command (`soarMcpServer.ts`'s handler is one shared `case` block for both tool names), split into two tools purely so the LLM picks the right one by name instead of guessing from one overloaded description: `_working_memory` for identifiers/WME patterns/goal-stack (target e.g. `"(S1 ^operator)"`), `_production` for rule text/production listings (target a production name, or the `--all`/`--user`/etc. type-filter flags with no target). Tool descriptions spell out concrete recipes for each since local models were passing invalid targets like bare `0` or `^operator`. `structuredOutput` defaults to `true` (requests SML `output="structured"` and adds a `names` array — every identifier/timetag in the result — to the response alongside `output` text; typically empty for `_production` results); pass `structuredOutput: false` for plain `output="raw"` text only. Plumbed through `DebugEvalInput.structuredOutput` → `SoarMcpCore.debugEval`/`runSmlCmdline` → `SmlSocketClient.call({ output: 'structured' })`. Covered by `test/mcp/helpers/print-structured-output.test.ts` (fake framed-XML TCP kernel; exercises `SoarMcpCore.debugEval` directly, not the tool-name split).
  - `agent_runtime_cli_print_pattern` — dedicated, structured-argument tool for the kernel's `print (id ^attribute value)` WME-pattern query (`Soar-kernel/Core/CLI/src/cli_print.cpp`'s `read_pattern_and_get_matching_wmes`/`read_pattern_component`): each of `id`/`attribute`/`value` independently defaults to `*` (wildcard, matches anything) if omitted. `acceptable: true` appends the trailing `+` to search acceptable-preference WMEs instead of established ones. The handler (`soarMcpServer.ts`) builds the `(id ^attribute value[ +])` string from the four fields and calls the same `SoarMcpCore.debugEval` path as the other print tools — this is a structured front-end over the same underlying command, not a new kernel capability. Split out as its own tool (rather than leaving pattern queries as just one way to write `agent_runtime_cli_print_working_memory`'s `target`) because the wildcard-search power (searching every identifier in memory at once, not just one you already know) wasn't discoverable buried in one string parameter's description — a model never reached for it.
    - **Regex is NOT reliably usable here, despite the kernel command technically supporting it for `attribute`/`value`** (via `std::regex_match` in `read_pattern_component`) — confirmed by direct testing against a live kernel, not just reading the source. Two compounding lexer issues: (1) `Lexer::get_lexeme_from_string` (`Soar-kernel/Core/SoarKernel/src/parsing/lexer.cpp:821-860`), which classifies the *entire* target argument before any pattern-specific parsing happens, treats any two `|` characters anywhere in the string as symbolic-constant quote delimiters — so `|`-quoting a regex fragment (the kernel's own documented escape for regex metacharacters) *reclassifies the whole argument* as a literal symbol name and routes it away from pattern parsing entirely, never reaching `read_pattern_and_get_matching_wmes`. (2) Even without quoting, characters like `.`/`[`/`]` are real Soar lexer syntax to the pattern's own internal tokenizer, so unquoted regex metacharacters break mid-pattern tokenization ("Expected ')' to end wme pattern"). Net effect: there is no working path to pass a non-trivial regex through this command. The tool description and schema deliberately steer callers toward exact literals and `*` only.
